...
const htmlLeaderboardContainer = document.getElementById(
...
async function fetchLeaderboard(QuizID) {
  ...
  if (QuizID !== null) {
    ...
    const response = await fetch(
      "https://codecyprus.org/th/api/leaderboard?treasure-hunt-id=" +
        QuizID +
        "&sorted=true&limit=25"
    )
      .then((response) => response.json() /* Convert it from JSON */)
      .then((json) => {
        leaderboardResponse = json; /* Save in Variable Array */
      });
    ...
    if (leaderboardResponse.status === "OK") {
      ...
      const response = await fetch("https://codecyprus.org/th/api/list")
        .then((response) => response.json() /* Convert it from JSON */)
        .then((json) => {
          availableTH = json.treasureHunts; /* Save in Variable Array */
        });
      ...
      for (let s = 0; s < availableTH.length; s++) {
        if (availableTH[s].uuid === QuizID) {
          htmlContentTitle.innerHTML =
            "<h1> Leaderboard for: " + availableTH[s].name + "</h1>";
        }
      }
      ...
      for (let s = 0; s < leaderboardList.length; s++) {
        tableInner +=
          "<tr>" +
          "<td>" +
          leaderboardList[s].player +
          "</td>" +
          "<td>" +
          leaderboardList[s].score +
          "</td>" +
          "<td>" +
          MiltoTime(leaderboardList[s].completionTime) +
          "</td>" +
          "</tr>";
      }
